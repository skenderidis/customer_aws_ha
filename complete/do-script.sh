...
if [[ ${HTTP_CODE} -ne 202 ]]; then
  ...
  sleep 30
  echo "Poll F5 BIGIP every 10 seconds to get the status of the DO jobID. The ID is $id"
  ...
  while [ "$status" == "RUNNING" ] && [ $count -lt 15 ]; do
    ...
    echo "$response" > "${PREFIX}-status-$(date '+%Y%m%d-%H%M%S').json"
  ...
  echo "EXIT - Status code $status and count is $count"
  # When the loop exits, the status is "COMPLETED" or a timeout occurred
  if [ "$status" != "OK" ]; then
    echo "Timeout occurred after 15 retries."
    echo $response | jq .
